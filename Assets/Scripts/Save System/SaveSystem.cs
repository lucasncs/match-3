...
public static class SaveSystem
{
	...
	private static readonly string fileName = Application.persistentDataPath + "/SaveData.txt";
	private static readonly string secureFileName = Application.persistentDataPath + "/SaveData";
	private static byte[] bytes = ASCIIEncoding.ASCII.GetBytes("mtc3" + SystemInfo.deviceUniqueIdentifier.Substring(0, 4));
	...
	public static string FileName
	{
		get
		{
			return (securityModeEnabled ? secureFileName : fileName);
		}
	}
	...
	public static void Save()
	{
		if (hashTableChanged)
		{
			...
			StreamWriter fileWriter = File.CreateText((securityModeEnabled ? secureFileName : fileName));
			File.Delete((securityModeEnabled ? fileName : secureFileName));

			if (fileWriter == null)
			{
				Debug.LogWarning("SaveSystem::Flush() opening file for writing failed: " + fileName);
				return;
			}
	...
	{
		Debug.Log($"Loading data on {Application.persistentDataPath}");
		...
		if (File.Exists(secureFileName))
		{
			...
			wasEncrypted = true;
			ProcessData(Decrypt(fileReader.ReadToEnd()));
		}
	...
	private static void ProcessData(string data)
	{
		...
		{
			//In the old SaveData, a WriteLine was used to write to the file.
	...
	private static void Deserialize()
	{
		...
		foreach (string parameter in parameters)
		{
			...
			{
				Debug.LogWarning("SaveSystem::Deserialize() parameterContent has " + parameterContent.Length + " elements");
			}
		}
	}

	private static string EscapeNonSeperators(string inputToEscape, string[] seperators)
	{
	...
	private static string DeEscapeNonSeperators(string inputToDeEscape, string[] seperators)
	{
